...
class MessagesController extends Controller{
    ...
    //获取消息内容
    public function show($message_id, Request $request){
        ...
        // 如果传了user_id 则返回user_id对应是否已读
        if ($userId = $request->input('user_id')) {
            $targetStatus = TargetStatus::query()->where('message_id', $message['id'])
                ->where('target_id', $userId)->first();

            $message['status'] = $targetStatus ? $targetStatus->status : 'unread';
        }
